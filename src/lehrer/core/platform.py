...
class OpenedxPlatform:
    ...
    def collected(
        ...
        The ``custom_settings`` directory must follow this layout::
            ...
            │   ├── aqueduct.py
            │   └── models/
            │       └── aqueduct.py  ← generated by regenerate-aqueduct-settings
            ├── cms/
            │   ├── assets.py
            │   ├── i18n.py
            │   ├── aqueduct.py
        ...
        core — operators do not provide it.
        ...
        container = (
            ...
            # models/base.py    → ProductionSettingsMixin from lehrer core (not from custom_settings);
            #                     models/__init__.py created here to enable relative imports
            # lms/models/aqueduct.py → generated AqueductSettings(ProductionSettingsMixin) per service
    ...
    async def regenerate_aqueduct_settings(
        ...
        generate_script = "\n".join(
            [
                ...
                "    return '\\n'.join(lines) + '\\n'",
                "def _fix_path_types(code):",
                "    # Fields whose default is Path(...) must be annotated Path, not str.",
                "    # Without this the default gets coerced to a plain string by pydantic",
                "    # and any Django code that does `settings.PROJECT_ROOT / 'static'` fails.",
                "    return re.sub(",
                "        r'(\\s+\\w+): str = (Field\\(default=Path\\()',",
                "        r'\\1: Path = \\2',",
                "        code,",
                "    )",
                "for shim, out in [",
