...
def _mysql(sql: str, *, timeout: str = "30s") -> str | None:
    """Run ``sql`` as root inside the MariaDB pod; ``None`` if unreachable.
...
def _warn_on_uncollated_edxapp_database() -> None:
    ...
    ``characterSet`` and ``collate`` are immutable, so a cluster whose Database
    was generated by the MariaDB reconciler (on the CRD's utf8 defaults) rejects
    the manifest with an admission error naming a field and no remedy. Deleting
    the resource is safe now that it declares ``cleanupPolicy: Skip`` — but only
    the *replacement* carries that, so the existing one still has to go first,
    and its finalizer would drop edxapp on the way out.
    """
    character_set = capture(
        "kubectl",
        "--context",
        CONTEXT,
        "-n",
    ...
    if not character_set or character_set == EXPECTED_CHARACTER_SET:
        return
    print(
        f"\n!!! The edxapp Database resource was created on '{character_set}' —\n"
        "    the operator generated it before the manifest declared one. Its\n"
        "    characterSet and collate are immutable, so `lehrer dev start` will\n"
        "    fail on the mysql-database resource. Detach it from its finalizer\n"
        "    first, or deleting it drops edxapp:\n"
...
def db_collation(*, fix: bool = False, check_tables: bool = False) -> None:
    ...
    if not drifted:
        ...
        for schema in drifted:
            ...
            print(f"ALTER DATABASE {schema} -> {EXPECTED_COLLATION}")
        print(
...
def _report_corrupt_tables(schemas: list[str]) -> None:
    ...
    for schema in schemas:
        ...
        tables = [line.strip() for line in (listing or "").splitlines() if line.strip()]
        if not tables:
        ...
        for start in range(0, len(tables), 50):
            names = ", ".join(
                f"`{schema}`.`{table}`" for table in tables[start : start + 50]
            )
            out = _mysql(f"CHECK TABLE {names} QUICK", timeout="300s")
            problems.extend(
                f"    {parts[0]}: {parts[2]} — {parts[3]}"
                for parts in (line.split("\t") for line in (out or "").splitlines())
                if len(parts) == _CHECK_TABLE_COLUMNS and parts[2] != "status"
            )
        if not problems:
            print(f"{schema}: {len(tables)} tables, none reporting an error")
            continue
        print(f"{schema}: {len(problems)} problem(s) across {len(tables)} tables")
        print("\n".join(problems))
        print(
