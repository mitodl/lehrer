...
const SidebarAIDrawerCoordinator = ({ courseId }) => {
	const { isNewDiscussionSidebarViewEnabled } = useModel(
		"courseHomeMeta",
		courseId,
	);
	...
	const contextValue = isNewDiscussionSidebarViewEnabled
		? newContextValue
		: oldContextValue;
	...
	const shouldDisplayFullScreen =
		contextValue?.shouldDisplayFullScreen ?? false;
	...
	const handleAIDrawerMessage = useCallback(
		(event) => {
			...
			if (
				event.data?.type &&
				AI_DRAWER_MESSAGE_TYPES.includes(event.data.type)
			) {
