...
const DEFAULT_BUNDLE_PATH = "/learn/static/smoot-design/aiDrawerManager.es.js";

/**
 * Messages passed to the smoot-design bundle via buildTranslationFn.
 * Keys MUST match TRANSLATION_KEYS in smoot-design's translationKeys.ts.
 */
const bundleMessages = defineMessages({
	"aiChat.feedbackGood": {
		...
		description:
			"Tooltip and aria-label for the thumbs-up feedback button on AI chat messages",
	},
	"aiChat.feedbackBad": {
		id: "learning.aiDrawer.feedbackBad",
		defaultMessage: "Bad response",
		description:
			"Tooltip and aria-label for the thumbs-down feedback button on AI chat messages",
	},
	...
	"aiChat.noAssignmentsMessage": {
		...
		description:
			"Message shown when no problem sets are available for the AI tutor",
	},
	"aiChat.errorGeneric": {
	...
	"aiDrawer.videoEntryScreenTitle": {
		...
		description:
			"Default title on the AI chat entry screen when opened from a video block",
	},
	...
	"flashcards.count": {
		...
		description:
			'Aria-label indicating the current flashcard position (e.g. "Flashcard 2 of 5")',
	},
	"flashcards.previous": {
	...
	"entryScreen.problemInitialMessage": {
		...
		description:
			"Default assistant greeting when the AI drawer opens on a problem block",
	},
...
const wrapperMessages = defineMessages({
	...
	errorLoadingTitle: {
		...
		description:
			"Title before the error message when the AI chat fails to load",
	},
});
...
let loadPromise: Promise<{ init: AIDrawerInit }> | null = null;

const loadAIDrawerManagerBundle = (bundlePath: string) => {
	if (loadPromise) {
		return loadPromise;
	}
	loadPromise = import(
		// webpackIgnore prevents Webpack from bundling this external bundle.
		// DO NOT REMOVE — removing this breaks runtime loading of aiDrawerManager.
		/* webpackIgnore: true */
		/* @vite-ignore */
		bundlePath
	)
		.then((module: { init?: AIDrawerInit }) => {
			if (!module?.init) {
				throw new Error("Bundle loaded but init function not found");
			}
			return module as { init: AIDrawerInit };
		})
		.catch((error: Error) => {
			loadPromise = null;
			throw new Error(
				`Failed to load aiDrawerManager bundle from ${bundlePath}: ${error.message}`,
			);
		});
	return loadPromise;
};
...
const AIDrawerManagerSidebar: FC = () => {
	const intl = useIntl();
	const { lmsBaseUrl, commonAppConfig } = useSiteConfig();
	const bundlePath =
		((commonAppConfig as Record<string, unknown>)?.mitolAIDrawer as
			| string
			| undefined) ?? DEFAULT_BUNDLE_PATH;
	 ...
	 * intl.formatMessage() at call time, enabling full ICU message format support.
	 */
	const buildTranslationFn =
		...
		(
			...
			return i.formatMessage(descriptor, vars);
		};
	...
	useEffect(() => {
		...
		loadAIDrawerManagerBundle(bundlePath)
			.then((aiDrawerManager) => {
				...
				try {
					const initResult = aiDrawerManager.init(
						{
							...
							variant: "slot",
							transformBody: (messages: Array<{ content: string }>) => ({
					...
					if (isMounted) {
						setInitError(
							getErrorMessage(error, "Failed to initialize AIDrawerManager"),
						);
						setIsLoading(false);
					}
				}
			...
			.catch((error: unknown) => {
				if (isMounted) {
					setInitError(
						getErrorMessage(error, "Failed to load AIDrawerManager bundle"),
					);
					setIsLoading(false);
				}
			});
		...
		>
				...
				>
					<div className="spinner-border spinner-border-sm" role="status">
						<span className="sr-only">
							{intl.formatMessage(wrapperMessages.loadingSrOnly)}
						</span>
					</div>
				</div>
			)}
			{initError && (
				<div className="alert alert-danger" role="alert" aria-live="assertive">
					<strong>
						{intl.formatMessage(wrapperMessages.errorLoadingTitle)}
					</strong>{" "}
					{initError}
				</div>
			)}
