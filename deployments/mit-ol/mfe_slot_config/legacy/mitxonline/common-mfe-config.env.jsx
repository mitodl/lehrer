...
const addFooterSubSlotsOverride = (config) => {
	...
	const footerLogo = (
		<Logo
			imageUrl={configData.LOGO_TRADEMARK_URL}
			destinationUrl={process.env.MIT_BASE_URL}
		/>
	);
...
const getUserMenu = ({ includeDashboard = false } = {}) => {
	...
	const userMenuLinkMessages = {
		...
		account: {
			id: "header.user.menu.account.setting",
			defaultMessage: "Settings",
		}, // Custom key for MITx Online
	...
	let dashboardURL = process.env.MIT_LEARN_BASE_URL
		? `${process.env.MIT_LEARN_BASE_URL}/dashboard`
		: "https://learn.mit.edu/dashboard";
	if (isMITxOnlineCourse()) {
		dashboardURL = configData.MARKETING_SITE_BASE_URL
			? `${configData.MARKETING_SITE_BASE_URL}/dashboard/`
			: "https://mitxonline.mit.edu/dashboard/";
	}
...
const modifyLogoHref = (widget) => {
	if (isMITxOnlineCourse()) {
		widget.content.href = configData.MARKETING_SITE_BASE_URL
			? `${configData.MARKETING_SITE_BASE_URL}/dashboard/`
			: "https://mitxonline.mit.edu/dashboard/";
	} else {
...
const SecondaryMenu = () => {
	...
	return (
		...
		>
			<a href={dashboardURL} className="dashboard-btn">
				<FormattedMessage
